...
void follow_segment()
{
  ...
  while(1)
  {
    ...
    int power_difference = 0.1*proportional;
...
void follow_segment_turn(unsigned char dir){
  ...
  while(1)
  {
   ...
   // 如果黑線在機器人正中間"proportional"變數應該要為0 
    ...
    // 計算兩個馬達輸出的差值。如果此項為正值機器人將會左轉彎，反之亦然。
    int power_difference = 0.225*proportional;
...
// path變數會存放機器人走過的路徑。以字元的方式儲存，有一下四種。
// 'L' 表示左轉
// 'R' 表示右轉
// 'S' 表示直走
// 'B' 表示迴轉
