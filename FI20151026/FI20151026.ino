...
unsigned int Step_counter=0; //計算經過幾次follow_segment函數循環，由此來換算走了多少的距離
  ...
  while(1)
  {
    ...
    else if (sensors[0] > 200 || sensors[4] > 200)
    {
      ...
      return;
        ...
        Step_counter++; //follow_segment經過一次加一 
...
int toward=1;         //設定初始方向
                      ...
                      // 負向x=nx=3
                      // 負向y=ny=4
int Nxy=1;            //地圖上的座標號碼(左下為1，右上為最大，依照整個迷宮的大小定義)
unsigned int Block_counter=0;   //換算用
void Blockcounter()             //計算3pi自走車走到的座標點
{
  if (Step_counter >= 200 && Step_counter < 400)  //200-400平均為一段黑線的Step_counter距離
  Block_counter=1;                                //所以將200-400設定為一個閥值
  else if (Step_counter >= 400)                   //大於400表示走過兩段無路口的直線
  Block_counter=2 ;                               //目前迷宮沒有設計大於兩段無轉彎路口的直線
  else
  Block_counter = 0;
  if (toward == 1)                                //目前迷宮的座標設計
      Nxy=Nxy+Block_counter;                      //33 34 35 36 37 38 39 40
    else if(toward == 2)                          //25 26 27 28 29 30 31 32
      Nxy=Nxy+Block_counter*8;                    //17 18 19 20 21 22 23 24
    else if(toward == 3)                          // 9 10 11 12 13 14 15 16
      Nxy=Nxy-Block_counter;                      // 1  2  3  4  5  6  7  8
    else
      Nxy=Nxy-Block_counter*8; 
}


void toward_way(unsigned char dir)                //依照自走車轉向來改變toward方向
{
 if (dir == 'R')
  {    if (toward==1) 
        toward = 4;
...
#include <avr/pgmspace.h>
char path[200] = "";
unsigned char path_length = 0; // the length of the pathrows
unsigned char Memory[50][5]={'0'};
int mx = 0;
int k = 0;

// Displays the current path on the LCD, using two  if necessary.
void display_path()
{
...
char dirr = '0';
// 這個函數會決定機器人在交叉口要轉哪個方向。本程式使用的是向心法則。
...
{ 
  ...
  { 
    ...
    else if (found_left && found_straight)
    { 
      if (toward==1)
      {
          Memory[mx][1]='L';
